...
def get_cond_model(opt, cond_type: ExtraCondition):
    if cond_type == ExtraCondition.sketch:
        ...
        version ='/home/pnp/SINE/clip-vit-large-patch14'
        ...
        return model
    elif cond_type == ExtraCondition.inpaint:
        return None
    else:
        raise NotImplementedError
...
def get_cond_canny(opt, cond_image, cond_inp_type='image', cond_model=None):
    ...
    if cond_inp_type == 'canny':
        canny = img2tensor(canny)[0:1].unsqueeze(0) / 255.##和depth channel不同的原因
...
def get_cond_inpaint(opt, cond_image,cond_inp_type='image', cond_model=None,mask=None,mask_blur=5.0):
    if isinstance(cond_image, str):
        img_raw = cv2.imread(cond_image)
        input_mask = cv2.imread(mask)
    else:
        img_raw = cv2.cvtColor(cond_image, cv2.COLOR_RGB2BGR)
