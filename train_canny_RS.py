...
def parsr_args():
    ...
    return opt
#废弃的自定义collate_fn，无法解决H!=W时的问题
# def collate_fn(batch):
#     # return {
#     #   [{'sentence',x['sentence']} for x in batch],
#     #   [{'im':x['im']} for x in batch],
#     #   [{'canny' : x['canny']} for x in batch]
#     # }#不对，这样直接是不符合loader的list要求了
#     # return batch#这样之后读出来还是有batch_idx
#     return {
#         'sentence':[x['sentence'] for x in batch],
#         'im':[x['im'] for x in batch],#
#         'canny':[x['canny']for x in batch]
...
def main():
    ...
    # dataset
    # train_dataset = CannyDataset('datasets/laion_depth_meta_v1.txt')
